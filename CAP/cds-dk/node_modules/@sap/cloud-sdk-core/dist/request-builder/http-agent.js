...
var certificateOptions = function (destination) { return function (options) {
    if (destination.keyStoreName && destination.keyStorePassword) {
        ...
        return __assign({}, options, { pfx: Buffer.from(certificate.content, 'base64'), passphrase: destination.keyStorePassword });
    }
    return options;
}; };
...
function proxyAgent(proxyHost, proxyPort) {
    return new HttpProxyAgent("http://" + proxyHost + ":" + proxyPort);
}
