...
function getResourcePathForKeys(keys, entityConstructor) {
    ...
    if (keys && Object.keys(keys).length) {
        ...
        var byKey = Object.entries(keys)
            ...
            if (validateNonNullableValue(value)) {
                return keyToOData(key, value, entityConstructor);
            }
            else {
                throw new Error("The value " + value + " for the key " + key + " is not nullable. Please set a value.");
            }
        })
            .join(',');
...
function validateNonNullableValue(value) {
