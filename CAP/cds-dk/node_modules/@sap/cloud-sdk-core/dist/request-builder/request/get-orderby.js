...
function getOrderByExpressionForOrderLink(orderBy, parentFieldNames) {
    if (parentFieldNames === void 0) { parentFieldNames = []; }
    return getODataOrderByExpressions(orderBy.orderBy, parentFieldNames.concat([orderBy.link._fieldName])).join(',');
}
function getOrderByExpressionForOrder(orderBy, parentFieldNames) {
    if (parentFieldNames === void 0) { parentFieldNames = []; }
    return [parentFieldNames.concat([orderBy._fieldName]).join('/'), orderBy.orderType].join(' ');
}
