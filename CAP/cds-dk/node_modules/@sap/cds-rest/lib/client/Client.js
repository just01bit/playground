...
const createEnvironmentVariable = (name, kind, credentials) => {
  if (!credentials) {
  ...
  const destination = { name, ...credentials }

  if (!process.env.destinations) {
    process.env.destinations = JSON.stringify([destination])
  } else {
    const destinationEnv = JSON.parse(process.env.destinations)
    destinationEnv.push(destination)
    process.env.destinations = JSON.stringify(destinationEnv)
  }
}
...
class RestClient {
  ...
  run (...args) {
    if (typeof this.destination !== 'string') {
      // TODO: How to find out we use the destination service
      if (this.destination === true) {
        this.destination = findServiceName(this._csn, this.name, this.options)
      } else {
        this.destination = findServiceName(this._csn, this.name, this.options)
        createEnvironmentVariable(this.destination, this.kind, this.options.credentials)
      }
    }
  ...
  execute (query, data) {
    ...
    if (typeof query === 'object') {
      try {
        ...
        reqOptions.url = encodeURI(queryObject.path
  ...
  _run (reqOptions) {
    return executeHttpRequest({ destinationName: this.destination, jwt: this._jwt }, reqOptions).then(response => {
