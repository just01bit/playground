...
const _isImmutableReadOnly = (element, event) => {
  if (element['@Core.Immutable'] && !element['@cds.on.update']) {
    return event === 'UPDATE' || event === 'PATCH'
  }
}
...
const getDeepSelect = (context, definitions, draft) => {
  ...
  const cols = Object.keys(context.target.elements).filter(
    k => !DRAFT_COLUMNS.includes(k) && context.target.elements[k].type !== 'cds.Association'
  )
  ...
  if (context.event) {
    ...
    cqn = context.statements.SELECT.from(context.target).columns(...cols)
  }
  ...
  const comps = cols.filter(k => context.target.elements[k].type === 'cds.Composition')
  if (comps.length > 0) {
