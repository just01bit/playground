...
class Service extends Client {
  ...
  _triggeredByDraftActivate (context) {
    return (context.event === 'CREATE' || context.event === 'UPDATE') && context.target['@odata.draft.enabled']
  }
  ...
  async processEvent (context) {
    ...
    if (MOD.includes(context.event) && !this._triggeredByDraftActivate(context)) {
  ...
  _addRunIfPrimarySessionExists (context) {
    ...
    context.run = (...args) => {
      ...
      if (
        ...
        !(context._.shared && context._.shared.connections && context._.shared.connections.has(cds.db))
      ) {
