...
const _valuesToOrConditions = (columnRef, op, values) => {
  ...
  for (const value of values) {
    ...
    const columnRefCopy = { ref: [...columnRef.ref], as: columnRef.as }
    arr.push(columnRefCopy, op, { val: value })
  }
...
const _ensureTableAlias = (ref, aliases, model, hasExpand) => {
...
const _enhanceAnnotationSubSelect = (select, model, targetName, hasExpand) => {
  if (select.from && select.from.ref) {
    select.from.ref[Symbol.for('sap.cds.FROM_ANNOTATION')] = true
  }
  if (select.where) {
    for (const v of select.where) {
      if (v.ref && select.from.ref) {
        _ensureTableAlias(v.ref, [targetName, select.from.ref[0]], model, hasExpand)
      }
    }
  }
}
...
const _enhanceAnnotationWhere = (query, where, model) => {
  ...
  for (const w of where) {
    if (w.ref) {
      _ensureTableAlias(w.ref, [query._target.name], model, hasExpand)
    } else if (w.SELECT) {
      _enhanceAnnotationSubSelect(w.SELECT, model, query._target.name, hasExpand)
    }
  }
}
