...
const _getSelectCQN = (model, { query, statements, target: { name } }, checkUser = true) => {
...
const onPatchDraft = ({ model } = {}) => async context => {
  ...
  let result = await context.run(_getSelectCQN(model, context))
  ...
  result = await context.run(_getSelectCQN(model, context, false))
  if (result.length === 0) {
    context.reject(404)
  }
