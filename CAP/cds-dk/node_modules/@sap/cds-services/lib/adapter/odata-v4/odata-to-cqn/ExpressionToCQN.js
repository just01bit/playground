...
class ExpressionToCQN {
  ...
  _member (expression) {
    ...
    return { ref: this._getMemberRecursively(pathSegments) }
  }
  ...
  _throwIfSecondArgumentIsNotSupported (param, method) {
    if (param.getKind() === ExpressionKind.METHOD) {
      throw getFeatureNotSupportedError(
        `Function as second parameter in method "${method}" of $filter or $orderby query options`
      )
    }

    if (param.getKind() === ExpressionKind.MEMBER) {
      throw getFeatureNotSupportedError(
        `Property as second parameter in method "${method}" of $filter or $orderby query options`
      )
    }
  }
  ...
  _method (expression, operator) {
    ...
    switch (expression.getMethod()) {
      case MethodKind.CONTAINS:
        this._throwIfSecondArgumentIsNotSupported(expression.getParameters()[1], MethodKind.CONTAINS)
        return this._like(parameters[0], { val: `%${parameters[1].val}%` }, operator)
      case MethodKind.STARTSWITH:
        this._throwIfSecondArgumentIsNotSupported(expression.getParameters()[1], MethodKind.STARTSWITH)
        return this._like(parameters[0], { val: `${parameters[1].val}%` }, operator)
      case MethodKind.ENDSWITH:
        this._throwIfSecondArgumentIsNotSupported(expression.getParameters()[1], MethodKind.ENDSWITH)
        return this._like(parameters[0], { val: `%${parameters[1].val}` }, operator)
      case MethodKind.NOW:
  ...
  _binary (expression) {
    ...
    switch (operator) {
      ...
      case BinaryOperatorKind.NE:
        return right && right.val === null ? [left, 'is not null'] : ['(', left, '!=', right, 'or', left, 'is null)']
