...
const Config = [ class {
  ...
  constructor (_context, _home) {
    ...
    if (_context === 'cds')  this.add (DEFAULTS, defaults)
    if (!_home)  return
...
//
  ...
  _add_to_process_env (cwd, filename) {
    ...
    const json = file.endsWith('.json') ? _readJson(file) : _readProperties(file)
    if (!json)  return
...
function _process_env4 (prefix) { // NOSONAR
  ...
  for (let p in env) if (p.startsWith(PREF_) && p !== PREF_+'ENV' && p !== PREF_+'CONFIG') {
    ...
    let val = env[p]
    if (val && val[0] === '{') try { val = JSON.parse(val) } catch(e) {/* ignored */}
...
function _readProperties (file) {
  return Properies.read(file)
}
