...
class HanaModuleBuilder extends BuildTaskHandler {
    ...
    async build() {
        ...
        const plugins = await this._compileToHana(model, dest)
    ...
    async _compileToHana(model, dest) {
        ...
        for (let [content, key] of result) {
            ...
            const relDest = path.join(GENERATED_SOURCES_FOLDER, key.file ? key.file : key.name + key.suffix)
            this._result.hana.push(relDest)
            const absDest = path.join(dest, relDest)
            if (!this.hasBuildOption(BUILD_OPTION_OUTPUT_MODE, OUTPUT_MODE_RESULT_ONLY)) {
                promises.push(this.write(content).to(absDest))
            }
        }
        await Promise.all(promises)
    ...
    async _writeHdiConfig(plugins) {
        const hdiConfig = path.join(this.task.dest, GENERATED_SOURCES_FOLDER, FILE_NAME_HDICONFIG)
    ...
    async _writeHdiNamespace() {
        ...
        const hdiNamespace = path.join(this.task.dest, GENERATED_SOURCES_FOLDER, FILE_NAME_HDINAMESPACE)
