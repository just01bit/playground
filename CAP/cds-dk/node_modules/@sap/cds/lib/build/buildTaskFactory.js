...
class BuildTaskFactory {
    ...
    _resolveTask(task, buildOptions) {
        ...
        if (!resolvedTask.use) {
            if (resolvedTask.for) {
                resolvedTask.use = "@sap/cds/lib/build/" + resolvedTask.for
            } else {
        ...
        tasks.forEach(task => {
            switch (task.for) {
                ...
                case BUILD_TASK_HANA:
                    ...
                    // add the fiori folders to the db build task, e.g. required for draft table generation
                    task.options.model = task.options.model.concat(fioriAppModelPaths)
                    break
    ...
    _createTaskFromYamlModule(projectPath, module) {
        ...
        if (types[module.type] && typeof module.path) {
            ...
            switch (types[module.type]) {
                ...
                case BUILD_TASK_FIORI: {
                    ...
                    if (fs.existsSync(manifestPath)) {
                        ...
                        this.logger.log(`[cds] - UI module ${module.name} does not contain a manifest.json, skipping module`)
                    }
                    break
                }
            }
            if (DEBUG) {
                if (task) {
                    this.logger.log(`[cds] - Creating build task for yaml module definition [${module.name}], type [${module.type}], path [${module.path}]`)
                } else {
                    this.logger.log(`[cds] - Skipping module [${module.name}] of type [${module.type}]`)
                }
            }
        }
    ...
    _createTasksFromCdsConfig(projectPath) {
        ...
        if (srv) {
            ...
            if (fioriTasks.length > 0) {
                ...
                dbOptions.model = dbOptions.model.concat(appDirs)
