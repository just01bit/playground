...
function transformForHanaWithCsn(inputModel, options) {
  ...
  // Ensure that there is only one draft root artifact per service
  forEachDefinition(csn, ensureOneDraftRootPerService);
  ...
  forEachDefinition(csn, (artifact) => {
    if(artifact.includes) delete artifact.includes;
  });
  ...
  forEachDefinition(csn, (artifact, artifactName) => {
    if (artifact.kind === 'namespace') delete csn.definitions[artifactName];
  });
  ...
  function ensureOneDraftRootPerService(artifact, artifactName){
  ...
  function handleDBChecks(artifact, artifactName){
    ...
    forEachMemberRecursively(artifact, (member, memberName, prop, path) => {
      ...
      if (member.masked) delete member.masked;
  ...
  */
  ...
  function transformCommon(obj, objName, path) {
    ...
    */
    ...
    if(obj.localized) delete obj.localized;
  ...
  function flattenFKs(assoc, assocName, path) {
    for (let i = 0; i < csn.definitions[path[1]].elements[assocName].keys.length; i++) {
      const { art } = inspectRef(path.concat(['keys', i]));
      const ref = csn.definitions[path[1]].elements[assocName].keys[i].ref;
      if (isStructured(art)) {
        ...
        Object.keys(flat).forEach(flatElemName => {
          ...
          if(clone.as){
            ...
            clone.as = clone.as + flatBaseName;
          }
          if (clone.ref) {
            clone.ref[clone.ref.length - 1] = flatElemName;
          }
        ...
        if(csn.definitions[path[1]].elements[assocName].keys[i].ref && !csn.definitions[path[1]].elements[assocName].keys[i].as){
          setProp(csn.definitions[path[1]].elements[assocName].keys[i], '$inferredAlias', true);
          csn.definitions[path[1]].elements[assocName].keys[i].as = csn.definitions[path[1]].elements[assocName].keys[i].ref[csn.definitions[path[1]].elements[assocName].keys[i].ref.length-1];
        }
      }
    }
  ...
  function cloneAndExtendRef(key, base, ref) {
    ...
    if (key.ref) {
      ...
      clone.ref = clone.ref.concat(key.ref); //  pathDelimiter + key.ref.join(pathDelimiter);
    }
   ...
   * 
   * @param {any} artifact Artifact to check
   * @param {any} artifactName Name of the artifact
   */
                ...
                for(let i = links.length -1; i >= 0; i--){
                  ...
                  if(link.art && link.art.target && link.art.$managed){
                    ...
                    if(!ref.slice(i).some(refElement => typeof refElement !== 'string')){
                      ...
                      const source = findSource(links, i-1);
                      if(source && source.elements[sourceElementName]){
                      // If <managed assoc>_<field> is in the elements of the source
                      // rewrite the reference accordingly.
                      // This way, foreign keys of the managed assoc are addressable in the on-condition
                        refOwner.ref = [...ref.slice(0, i), sourceElementName];
                      }
                    }
                  }
                }
