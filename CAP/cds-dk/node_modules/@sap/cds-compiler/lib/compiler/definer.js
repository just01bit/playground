...
function getDefinerFunctions( model ) {
    ...
    function checkReserved() {
      if (absolute === 'cds' ||
        ...
        return !model.definitions[absolute];
               ...
               art.$inferred !== '' && // temp for manual localization views with --beta
  ...
  function initArtifacts( construct, parent, block, collectExts,
    ...
    function setName( art, localName ) {
      ...
      if (block !== model.$internal &&
        ...
        const manualLocalizedHack = (options.beta || options.betaMode) &&
              art.name && art.name.path && art.name.path[0] && art.name.path[0].quoted;
        if (!manualLocalizedHack && block && block.$frontend !== 'json') {
          message( null, art.name.location, art, {}, 'Info',
                   'This definition in the namespace "localized" is ignored' );
        }
        if (!manualLocalizedHack) {
          art.$inferred = 'LOCALIZED-IGNORED';
          return;
        }
        art.$inferred = '';
  ...
  function processLocalizedData( art ) {
    const draftAnno = art['@odata.draft.enabled'];
    const draftEnabled = (draftAnno && (draftAnno.val === undefined || draftAnno.val) &&
      (options.betaMode || options.beta));

    const textsName = `${ art.name.absolute }_texts`;
    const textsEntity = model.definitions[textsName];
    const localized = localizedData( art, textsEntity, draftEnabled );
    if (!localized)
      return;
    ...
    createTextsEntity( art, textsName, localized, draftEnabled );
    addTextsAssociations( art, textsName, localized );
  }

  function localizedData( art, textsEntity, draftEnabled ) {
    let keys = 0;
    const textElems = [];
    ...
    if (textsEntity) {
      ...
      if (textsEntity.kind !== 'entity' || textsEntity.query ||
          ...
          (draftEnabled && art.elements.ID_Texts)) {
  ...
  function createTextsEntity( base, absolute, textElems, draftEnabled ) {
    const elements = Object.create(null);
    ...
    for (const orig of textElems) {
      ...
      if (orig.key && orig.key.val) {
        ...
        // FIXME: Don't know if we still need this annotation to suppress foreign keys
        // in containment mode
  ...
  function createLocalizedDataView( base, absolute, textElems ) {
    if (model.definitions[absolute]) // temp possible with --beta and @cds.localized.manual
      return model.definitions[absolute];
    const { location } = base.name;
    /** @type {object[]} */
