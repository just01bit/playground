        ...
        for (let index = pathSegments.length - 1; index >= 0; index--) {
            ...
            switch (kind) {
                ...
                case ResourceKind.NAVIGATION_TO_MANY: {
                    ...
                    } else if (segment.getTarget()) {
                        const name = segment.getTarget().getName();
                        result.unshift(name + keys);
                        isFinal = true;
                        // hack that signals to the caller that '/$entity' must not be appended
                        if (edm.getEntityContainer().getSingleton(name)) isOnlyTyped = true;
                    } else {
                        isOnlyTyped = true;
                    }
