...
class OdataRequest {
     ...
     * and if these headers are not containing only '*' in the case of HTTP PATCH or HTTP PUT to control
     * upsert behavior. A GET request with an If-Match header value of '*' is also not conditional.
     * @returns {boolean} whether this request is conditional
     */
    isConditional() {
        if (this._isConditional === null) {
            const method = this.getMethod();
            const ifMatch = this.getHeader(HeaderNames.IF_MATCH.toLowerCase());
            const ifNoneMatch = this.getHeader(HeaderNames.IF_NONE_MATCH.toLowerCase());
            this._isConditional =
                !(method === HttpMethods.GET && ifMatch && ifMatch.trim() === '*' && !ifNoneMatch)
                && !((method === HttpMethods.PATCH || method === HttpMethods.PUT)
                    && (ifMatch && ifMatch.trim() === '*' || ifNoneMatch && ifNoneMatch.trim() === '*'))
