...
class ConditionalRequestValidator {
    ...
    preValidate(ifMatch, ifNoneMatch, method, isConcurrentResource) {
        if (isConcurrentResource) {
            ...
            // The only allowed value in these cases is '*'.
            if (method === HttpMethods.PATCH || method === HttpMethods.PUT) {
                if (ifMatch && ifMatch.trim() !== '*'
                    || ifNoneMatch && ifNoneMatch.trim() !== '*') throw new PreconditionFailedError();
            } else if (method !== HttpMethods.GET || ifMatch && ifMatch.trim() !== '*' || ifNoneMatch) {
                // A GET request with an If-Match header value of '*' is allowed, but otherwise
                // concurrency-control headers are not expected here.
     ...
     * @returns {boolean} if the result of the validation is "not modified", otherwise false
    ...
    validateEtag(ifMatch, ifNoneMatch, method, kind, etag) {
        ...
        if (ifMatch && !ifMatchEtags.includes(etag) && !ifMatchEtags.includes('*')) {
        ...
        return Boolean(
            ...
            && (!ifMatch || ifMatchEtags.includes('*') || ifMatchEtags.includes(etag))
