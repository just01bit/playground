...
class ResourceJsonDeserializer {
    ...
    deserializeEntity(edmType, value, expand, additionalInformation) {
        try {
            ...
            this._deserializeStructuralType(edmType, data, false, false, expand, additionalInformation);
            return data;
        } catch (e) {
    ...
    deserializeEntityCollection(edmType, value, expand, additionalInformation) {
        ...
        try {
            for (let entityValue of tempData) {
                this._deserializeStructuralType(edmType, entityValue, true, false, expand, additionalInformation);
            }
            return tempData;
        } catch (e) {
            if (e instanceof DeserializationError) throw e;
     ...
     * @private
     */
    _deserializeStructuralType(edmType, structureValue, isNested, isDelta, expand, additionalInformation) {
        ...
        for (const [name, value] of Object.entries(structureValue)) {
            if (name.includes('@')) {
                this._deserializeAnnotation(edmType, structureValue, name, value, isNested, isDelta,
                ...
                switch ((edmProperty.getEntityType ? edmProperty.getEntityType() : edmProperty.getType()).getKind()) {
                    case EdmTypeKind.COMPLEX:
                        this._deserializeComplex(edmProperty, value);
                        break;
                    case EdmTypeKind.ENTITY:
                        this._deserializeNavigation(edmProperty, value, expand, additionalInformation);
                        break;
                    default:
     ...
     * @private
     */
    _deserializeNavigation(edmProperty, value, expandArray, additionalInformation) {
        ...
        if (edmProperty.isCollection()) {
            for (let entity of value) {
                this._deserializeStructuralType(type, entity, true, false, newExpandOptionArray, additionalInformation);
            }
        } else {
            this._deserializeStructuralType(type, value, true, false, newExpandOptionArray, additionalInformation);
        }

        // Attach the sub-expand to its parent only if there is at least one path segment available in the child expand.
        // In this case at least one entity inside the collection has another navigation property.
    ...
    _deserializeComplex(edmProperty, propertyValue) {
        if (edmProperty.isCollection()) {
            for (let value of propertyValue) {
                this._assertPropertyNullable(edmProperty, value);
                if (value !== null) this._deserializeStructuralType(edmProperty.getType(), value, true);
            }
        } else {
            this._assertPropertyNullable(edmProperty, propertyValue);
            if (propertyValue !== null) this._deserializeStructuralType(edmProperty.getType(), propertyValue, true);
        }
    }
    ...
    _deserializeDelta(edmType, propertyName, value, additionalInformation) {
        ...
        for (const entity of value) {
            this._deserializeStructuralType(innerEdmType, entity, true, true, [], additionalInformation);
        }
    }
