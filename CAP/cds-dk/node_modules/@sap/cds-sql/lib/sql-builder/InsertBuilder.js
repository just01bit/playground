...
class InsertBuilder extends BaseBuilder {
  ...
  _entries (annotatedColumns) {
    const columns = []
    const valuesArray = []
    const flattenColumnMap = this._getFlattenColumnMap(this._obj.INSERT.entries[0])
  ...
  _getFlattenColumnMap (entry, prefix) {
    const res = new Map()
    ...
    Object.keys(entry).forEach(key => {
      ...
      if (
        ...
        const resInternal = this._getFlattenColumnMap(entry[key], prefixKey)
