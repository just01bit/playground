...
class JoinCQNFromExpanded {
  ...
  _isCompToOne (activeTable, target) {
    return activeTable && target && target.type === 'cds.Composition' && target.is2one
  }
  ...
  _addJoinAndElements ({ column, entity, readToOneCQN, toManyTree, parentAlias }) {
    ...
    // special case of composition to one requires additional LEFT JOIN and CASE for HasDraftEntity
    const compToOne = this._isCompToOne(readToOneCQN[IS_ACTIVE], entity.elements[column.ref[0]])
